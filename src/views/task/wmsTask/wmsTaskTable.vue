...
*/
import { selectWmsTaskList } from '@/api/task/wmsTask'
...
export default {
  ...
  methods: {
    ...
    loadPageTableList () {
      ...
      selectWmsTaskList(this.parameterDto).then(response => {
        this.wmsTaskList = response.data
    ...
    //执行任务
    doTask (wmsTaskId) {
      doWmsTask(wmsTaskId).then(response => {
        alert(response.msg)
      })
    },
    //查看详情计划
