...
*/
import { selectCutTaskList, doCutTask } from '@/api/task/cutTask'
...
export default {
  ...
  methods: {
    ...
    loadPageTableList () {
      ...
      selectCutTaskList(this.parameterDto).then(response => {
        this.cutTaskList = response.data
    ...
    doTask (cutTaskId) {
      debugger
      doCutTask(cutTaskId).then(response => {
        alert(response.msg)
      })
    },
